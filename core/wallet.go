...
func FindWalletByPvtKey(pvtkey string) (*Wallet, error) {
	...
	wallit.PrivateKey.D = hextoint
	wallit.PrivateKey.PublicKey = GeneratePublicKey(hextoint)
	wallit.PublicKey = append(wallit.PrivateKey.PublicKey.X.Bytes(), wallit.PrivateKey.PublicKey.Y.Bytes()...)
	wallit.Real.Address = string(wallit.Address())
	wallit.Real.Pvtkey = fmt.Sprintf("%x", wallit.PrivateKey.D)
	wallit.Real.Pubkey = fmt.Sprintf("%x", wallit.PublicKey)
	if pvtkey == wallit.Real.Pvtkey {
		return wallit, nil
	}
	return wallit, err
...
func MakeWallet() *Wallet {
	...
	ree := RealWallet{}
	wall := Wallet{privateKey, publicKey, ree}

	ree = RealWallet{
		Pvtkey:  fmt.Sprintf("%x", privateKey.D),
		Pubkey:  fmt.Sprintf("%x", publicKey),
		Address: fmt.Sprintf("%s", wall.Address()),
	}
	wallet := Wallet{privateKey, publicKey, ree}
	return &wallet
}
